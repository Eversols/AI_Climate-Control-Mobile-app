...
const PestScreen1 = ({navigation, route}) => {
  ...
  const handlefarmOptions = i => {
    setCorpOptions(
      farmList[i].farmCrops.length > 0
        ? farmList[i].farmCrops.map(item => {
            return {id: item?.crop?.id, name: item?.crop?.name};
          })
        : [],
    );
  ...
  useEffect(() => {
    ...
    if (farmList.length > 0 && route?.params?.farm && route?.params?.crop) {
      ...
      const crops =
        farmList[0].farmCrops.length > 0
          ? farmList[0].farmCrops.map(item => ({
              id: item?.crop?.id,
              name: item?.crop?.name,
            }))
          : [];
